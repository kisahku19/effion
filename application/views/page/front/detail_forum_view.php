<script>
    ...
    function addRating(obj, id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>front/tambah_rating_forum",
...
<div class="row">
    <div class="col-md-8">
        <div id="portfolio-slider" class="flexslider">
            <ul class="slides">
                <center>
                    <img src="<?= base_url() ?>foto_forum/<?= $detail_forum->gambar ?>" alt="" style="width: 80%; height: 600px;" />
            </ul>
        </div>
    </div>
    <div class="col-md-4">
        <div class="portfolio-meta">
            <h5>Rating</h5>
            <div id='rate-0'>
                <input type='hidden' name='rating' id='rating' value='<?= $detail_forum->rating ?>'>
                <ul onMouseOut="resetRating(<?= $detail_forum->id_forum ?>)">
                    <?php
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        ...
                        <li class='<?= $selected ?>' onmouseover="highlightStar(this,<?= $detail_forum->id_forum ?>)" onmouseout="removeHighlight(<?= $detail_forum->id_forum ?>);" onClick="addRating(this, <?= $detail_forum->id_forum ?>)">&#9733;</li>
            ...
            <h5>Tanggal : <?= $detail_forum->tanggal ?></h5>
            <h5>Dibuat Oleh : <?= $detail_forum->nama ?></h5>
...
<div class="row">
        ...
        <?php if ($jumlah_comment > 0) { ?>
            <?php foreach ($list_comment as $value) { ?>
                <?php if ($value->id_parent_komentar_forum == 0) { ?>
                    ...
                    <form method="post" action="<?= base_url('komentar/balas_komentar_forum/' . $value->id_komentar_forum) ?>">
    ...
    <div class="col-md-12 comment-form">
        ...
        <form method="post" action="<?= base_url() ?>front/buat_komentar_forum">
            <div class="row">
                <div class="col-md-12">
